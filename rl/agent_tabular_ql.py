...
# pragma: coderesponse template
def tabular_q_learning(q_func:np.ndarray, current_state_1:int, current_state_2:int, action_index:int,
                       object_index:int, reward:float, next_state_1:int, next_state_2:int,
                       terminal):
...
def run_epoch():
    ...
    for _ in range(NUM_EPIS_TRAIN):
        run_episode(for_training=True) # modify Q-value.

    for _ in range(NUM_EPIS_TEST):
        rewards.append(run_episode(for_training=False)) # store the reward values.

    return np.mean(np.array(rewards)) # compute average rewards.
