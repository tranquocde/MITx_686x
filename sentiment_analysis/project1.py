...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    # Your code here
    # raise NotImplementedError
    z_s = feature_matrix @ theta + theta_0
    loss = np.maximum(1 - z_s * labels, np.zeros(len(labels)))
    return np.mean(loss)
...
def average_perceptron(feature_matrix, labels, T):
    ...
    theta = np.zeros(len(feature_matrix[0]))
    theta_0 = 0
    final_theta =np.zeros(len(feature_matrix[0]))
    final_theta_0 = 0
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            theta , theta_0 = perceptron_single_step_update(feature_matrix[i],labels[i],theta,theta_0)
    ...
    return final_theta / (T * len(feature_matrix)) , final_theta_0 / (T * len(feature_matrix))
...
def classify(feature_matrix, theta, theta_0):
    ...
    # Your code here
    # raise NotImplementedError
    # return (feature_matrix @ theta + theta_0 > 1e-7) * 2 - 1
...
def classifier_accuracy(
    ...
    return accuracy(train,train_labels),accuracy(val,val_labels)
