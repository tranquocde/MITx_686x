...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        vec_rec_linear = np.vectorize(rectified_linear_unit)
        vec_rec_linear_der = np.vectorize(rectified_linear_unit_derivative)
        # Calculate the input and activation of the hidden layer
        hidden_layer_weighted_input = self.input_to_hidden_weights * input_values + self.biases #(2,3)
        hidden_layer_activation = vec_rec_linear(hidden_layer_weighted_input) #(3,1)
        ...
        # Compute gradients
        output_layer_error = (activated_output - y) * output_layer_activation_derivative(output) # [1,1]
        hidden_layer_error = np.multiply((np.transpose(self.hidden_to_output_weights) * output_layer_error) , vec_rec_linear_der(hidden_layer_weighted_input)) # [3,1]

        bias_gradients = 1 * hidden_layer_error #(3,1)
